...
prettyPatternToSExpr pat = case pat of
  PatCon tname args repr targs _ _ _ _ ->
    SList [SAtom "pat-con", SAtom (show (pretty (getName tname))), SList (map prettyPatternToSExpr args), prettyConReprToSExpr repr, SList (map prettyTypeToSExpr targs)]
  PatVar tname p ->
